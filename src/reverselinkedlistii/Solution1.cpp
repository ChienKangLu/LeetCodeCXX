...
ListNode *Solution1::reverseBetween(ListNode *head, int left, int right) {
  ...
  ListNode *innerStart =
      start->next;           // the original inner start will be the inner tail
  ListNode *last = nullptr;  // <---- it doesn't matter, because the inner list
                             // will be concatenated again
  ...
  return dummy
      .next;  // <------- if whole list are reversed, then the head is changed
}
